/*********************************************
Kod stanowi uzupe�nienie materia��w do �wicze�
w ramach przedmiotu metody optymalizacji.
Kod udost�pniony na licencji CC BY-SA 3.0
Autor: dr in�. �ukasz Sztangret
Katedra Informatyki Stosowanej i Modelowania
Akademia G�rniczo-Hutnicza
Data ostatniej modyfikacji: 19.09.2023
*********************************************/

#include "opt_alg.h"

void lab0();
void lab1();
void lab2();
void lab3();
void lab4();
void lab5();
void lab6();

int main() {
	try
	{
		lab1();
	}
	catch (string EX_INFO)
	{
		cerr << "ERROR:\n";
		cerr << EX_INFO << endl << endl;
	}
	system("pause");
	return 0;
}

void lab0() {
	//Funkcja testowa
	double epsilon = 1e-2;
	int Nmax = 10000;
	matrix lb(2, 1, -5), ub(2, 1, 5), a(2, 1);
	solution opt;
	a(0) = -1;
	a(1) = 2;
	opt = MC(ff0T, 2, lb, ub, epsilon, Nmax, a);
	cout << opt << endl << endl;
	solution::clear_calls();

	//Wahadlo
	Nmax = 1000;
	epsilon = 1e-2;
	lb = 0;
	ub = 5;
	double teta_opt = 1;
	opt = MC(ff0R, 1, lb, ub, epsilon, Nmax, teta_opt);
	cout << opt << endl << endl;
	solution::clear_calls();

	//Zapis symulacji do pliku csv
	matrix Y0 = matrix(2, 1), MT = matrix(2, new double[2]{ m2d(opt.x),0.5 });
	matrix* Y = solve_ode(df0, 0, 0.1, 10, Y0, NAN, MT);
	ofstream Sout("symulacja_lab0.csv");
	Sout << hcat(Y[0], Y[1]);
	Sout.close();
	Y[0].~matrix();
	Y[1].~matrix();
}

void lab1() {
    fstream csvFile;

    const int numOfElements = 100;
    double minVal = -50, maxVal = 50;

    set<double> uniqueNums;

    default_random_engine randomEngine(random_device{}());
    uniform_real_distribution<double> realDistribution(minVal, maxVal);

    while (uniqueNums.size() < numOfElements)
        uniqueNums.insert(realDistribution(randomEngine));


    for (auto &uniqueNum : uniqueNums) {
//        Expansion

//    cout << "Expansion:\n";
        double x0 = uniqueNum, d = 0.75, alpha = 1.92;
        int Nmax = 1000;
        double *p = expansion(ff1T, x0, d, alpha, Nmax, NAN, NAN);
//    double *p = expansion(ff1T, 1, 0.5, 1.284, 1000, NAN, NAN);
//    cout << "a: " << p[0] << ", b: " << p[1] << '\n';

        try {
            csvFile.open(R"(C:\Users\Dell\Desktop\stuff\studia\semestr 5\Optymalizacja\project\data\data1\data1.csv)",
                         ios::app);
            csvFile << x0 << "," << p[0] << "," << p[1] << "," << solution::f_calls << ",";
            csvFile.close();
        }
        catch (error_code &errorCode) {
            cout << "Error code: " << errorCode << "\n";
        }
        solution::clear_calls();

//    Fibonacci

//    cout << "Fibonacci:\n";
        solution Fibonacci = fib(ff1T, p[0], p[1], 0.0001, NAN, NAN);
//    cout << "x: " << Fibonacci.x << "\ny: " << Fibonacci.y << "\n";
        try {
            csvFile.open(R"(C:\Users\Dell\Desktop\stuff\studia\semestr 5\Optymalizacja\project\data\data1\data1.csv)",
                         ios::app);
            csvFile << m2d(Fibonacci.x) << "," << m2d(Fibonacci.y) << "," << solution::f_calls << ",";
            csvFile.close();
        }
        catch (error_code &errorCode) {
            cout << "Error code: " << errorCode << "\n";
        }
        solution::clear_calls();

//    Lagrange

//    cout << "Lagrange:\n";
        solution Lagrange = lag(ff1T, p[0], p[1], 0.0001, 0.001, 1000, NAN, NAN);
//    if (Lagrange.flag)
//        cout << "x: " << Lagrange.x << "\ny: " << Lagrange.y << "\n";
//    else
//        cout << "x: " << Lagrange.x << "\nexit flag: " << Lagrange.flag << "\n";

        try {
            csvFile.open(R"(C:\Users\Dell\Desktop\stuff\studia\semestr 5\Optymalizacja\project\data\data1\data1.csv)",
                         ios::app);
            csvFile << m2d(Lagrange.x) << "," << m2d(Lagrange.y) << "," << solution::f_calls << "\n";
            csvFile.close();
        }
        catch (error_code &errorCode) {
            cout << "Error code: " << errorCode << "\n";
        }

        solution::clear_calls();
    }
    exit(0);
}

void lab2()
{

}

void lab3()
{

}

void lab4()
{

}

void lab5()
{

}

void lab6()
{

}
